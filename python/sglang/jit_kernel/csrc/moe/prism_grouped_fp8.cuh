...
enum Layout : int { ROWMAJOR = 0, KT_TILE8 = 1, ROWMAJOR_PT = 2, KT_TILE8_PT = 3 };
constexpr bool layout_is_tile(int l) { return (l & 1) != 0; }
constexpr bool layout_is_pt(int l) { return l >= ROWMAJOR_PT; }
...
inline void grouped_fp8_impl(
  ...
  RuntimeCheck(layout >= ROWMAJOR && layout <= KT_TILE8_PT, "grouped_fp8: unknown layout ", layout);
  if (tile) {
...
void grouped_fp8_cold(
  ...
  host::RuntimeCheck(layout == KT_TILE8 || layout == KT_TILE8_PT,
                     "grouped_fp8_cold: layout must be 1 (kt fp8 b128 tile) or 3 (kt fp8 per-tensor tile)");
...
void grouped_fp8_cold_gateup(
  ...
  host::RuntimeCheck(layout == KT_TILE8 || layout == KT_TILE8_PT,
                     "grouped_fp8_cold_gateup: layout must be 1 or 3");
