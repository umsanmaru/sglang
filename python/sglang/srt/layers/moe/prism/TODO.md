...
`idx-seq` = 연속 인덱스(밴드 등가), `idx-shuf` = 무작위 순열.

| 구성 | band | idx-seq | idx-shuf |
|---|---|---|---|
| hot gate/up (k=768, N=512) | 10.51 µs | **12.00 (1.14x)** | 12.13 (1.15x) |
| hot down (k=192, N=2048) | 11.09 µs | **11.27 (1.02x)** | 11.09 (1.00x) |
| warm gate/up (pinned/UVA, k=256) | 48.09 µs | **46.93 (0.98x)** | 47.53 (0.99x) |

bs 스윕 (hot gate 치수): 1.14x (bs≤4) → 1.20x (bs=8) → 1.35x (bs=32).
배치가 커질수록 distinct expert가 늘어 인덱스·x의 L2 재사용이 줄어든다.

**층당 순비용** (f_hot 0.375, bs=1): gate/up 2회 + down 1회 = **+3.2 µs/층**
→ 40층 **+0.13 ms/tok**, 18.84 ms/tok 대비 **+0.7%**.

### 여기서 확정된 것

**1. x 산란은 공짜다.** `idx-shuf`가 `idx-seq`와 차이 없다 (bs≤8에서 ±1%).
x는 한 행이 4 KB라 L1에 상주하므로 gather가 순차 읽기와 같은 값이다.
**티어 배치가 원본 순서를 지킬 이유가 없다** — planner가 자유롭게 섞어도 된다.

**2. 인덱스 비용은 지연이지 대역폭이 아니었다.** 첫 구현(내부 루프에서
`idx[r] → x[idx[r]]` 의존 로드)은 gate/up에서 **1.83배**였다. grid가 (8, 8) =
64블록뿐이라 188 SM에서 순수 지연 바운드이고, 의존 사슬이 1:1로 벽시계에 드러난다.
x 행을 블록당 1회 smem에 모으면(KTILE=2048) 내부 루프가 의존성 없는 smem 읽기가
되어 **1.83 → 1.14배**. down이 애초에 1.02배였던 이유도 같다 — 256블록에
루프가 1/4 길이라 지연이 이미 가려져 있었다.

**3. 스테이징은 인덱스 경로에만 건다.** 밴드 경로는 x를 순차로 읽어 끊을 사슬이
없고 syncthreads만 늘어 순손실이다 (10.5 → 12.3 µs). `if constexpr`로 갈랐다.

**4. warm은 인덱스가 완전히 공짜다** — PCIe 바운드(46~48 µs)라 커널 내부 비용이
묻힌다. hot과 반대로 warm 비율을 키우는 데 인덱스는 아무 대가가 없다.

**5. 가변 k[e]의 벽시계는 평균이 아니라 `max_e k[e]`에 비례한다** (총량 고정,
분포만 바꾼 실험):

| 분포 (평균 768) | max k[e] | 시간 | max 대비 |
|---|---|---|---|
| uniform | 768 | 12.09 µs | 1.00 |
| bimodal | 1152 (1.5x) | 17.30 µs | 1.43 |
| heavy | 2048 (2.67x) | 29.20 µs | 2.42 |

**planner 제약**: per-expert 예산을 몰아주면 그 층의 hot GEMV가 **가장 두꺼운
expert의 시간**을 낸다. 총량이 같아도 skew는 공짜가 아니다 — pair-native 커널은
pair당 블록이라 부하 분산이 wave 꼬리에 그대로 남는다. (완화하려면 K축을 grid로
쪼개 블록 간 리덕션이 필요한데, 그건 다른 커널이다.)
