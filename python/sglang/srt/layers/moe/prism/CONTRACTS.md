...
- score 변종은 `k2wl2` 하나로 고정했었다 (2026-08-24 사용자 결정) — **2026-09-05 개정:
  `{k2wl2, k1}` 둘**이고, 어느 것을 쓰는지는 **cold 커널 키가 함의한다**
  (`kernels.cold_sparsity_score`; plan.score와 어긋나면 cold_backend가 startup에서 즉사).
  - `k2wl2` (페어 커널 `kt_amx_bf16/kt_tile_k2_*`): 인접 입력채널 **페어**의 실제 에너지
    `sqrt(a0·x0² + a1·x1² + 2c·x0·x1)`이므로 열 노름 `wn`과 인접열 내적 `pair_dot` 둘 다를
    요구하고, 마스크는 `k/2` 비트다.
  - `k1` (per-k 커널 `kt_tile_k1_fp8b128`): **순수 활성값** `|x_k| >= thr`. 가중치 통계를
    쓰지 않아 calib 자산은 thr 곡선(`tg/tu/td`, per-k |x| 분위수)만이고, 마스크는 `k` 비트다.
    skip 단위가 k 한 행(64 B 라인)이라 페어 정렬 규칙은 이 커널에는 의미가 없지만, fp8의
    `k_align=128`이 그보다 거칠어 인덱스 규칙은 그대로 만족된다.
