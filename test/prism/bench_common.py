"""하드웨어 프로파일링 벤치의 공통 부품 — 두 스크립트가 공유하는 것만 둔다.

여기 있는 것은 셋이다:

1. **shape 어휘**: (E, top_k, hidden, inter)와 proj별 (K축, N, x행 규약). Plan을
   읽지 않는다 — 프로파일러는 "이 치수에서 이 연산이 몇 µs냐"만 묻고, 그 답이
   Plan을 만드는 입력이 되기 때문이다 (역방향 의존을 만들면 안 된다).

2. **타이머**: 커널 `reps`개를 CUDA graph 하나에 캡처해 replay 시간을 reps로
   나눈다. launch당 고정비(커널 launch ~2 µs)를 graph가 지우므로 남는 것이
   커널 자체의 시간이다. 다만 **같은 launch를 reps번 반복하면 L2-hot 시간**이
   나오므로, 호출자가 `launch(i)`의 i로 iteration마다 다른 expert를 태우게 했다
   (topk_ids 버퍼를 reps개 미리 만드는 패턴 — 두 스크립트가 다 그렇게 쓴다).

3. **sparsity 합성**: 요청한 sparsity를 **정확히** 실현하는 점수 재료.
   커널(GPU/CPU 양쪽)이 스스로 threshold를 계산하고 페어 점수와 비교하므로
   (계약 ①의 k2wl2), 원하는 마스크를 얻으려면 그 입력을 거꾸로 짜야 한다:

       imp²[j] = a[2j]·x0² + a[2j+1]·x1² + 2·c[j]·x0·x1
       keep[j] = imp[j] >= thr[e, round(s/grid)]

   x ≡ 1, c ≡ 0, a[2j] = a[2j+1] ∈ {1, 0}, thr 곡선을 상수로 채우면
   imp²[j] ∈ {2, 0}이고 thr² = 0.25이므로 keep[j] = (a[2j] == 1)이 된다.
   곡선이 상수라 격자 인덱스(→ s → 라우터 가중)가 무엇이든 조회값이 같다 —
   즉 **라우터 가중과 무관하게** 마스크가 결정되고, GPU와 CPU가 같은 마스크를
   본다 (양쪽이 같은 a/c/thr를 각자의 티어 행에 대해 받으므로).

계약상 sparse는 decode 전용(M=1)이다 (tiers.py / executor.py의 masking 조건).
"""
...
# 배수로 잡으면 로더의 타일 올림(real_rows)이 필요 없어 벤치가 패딩 없는
# 구성만 다룬다 — 패딩된 구성을 재려면 --k-rows를 직접 주면 된다.
K_STEP = 32
PAIR_GROUP = 2
...
def add_shape_args(p) -> None:
    ...
    p.add_argument("--hidden", type=int, default=2048, help="hidden_size")
    p.add_argument("--inter", type=int, default=768, help="intermediate_size")


def shape_from_args(a) -> Shape:
    if a.topk > 16:
        raise SystemExit("top_k <= 16 (커널의 per-thread slot 예산)")
    return Shape(experts=a.experts, topk=a.topk, hidden=a.hidden, inter=a.inter)
...
def split_rows(k_axis: int, frac: float, *, step: int = K_STEP) -> int:
    ...
    """
    if not 0.0 <= frac <= 1.0:
        raise SystemExit(f"fraction must be in [0, 1], got {frac}")
    if frac == 0.0:
        return 0
    if frac == 1.0:
        return k_axis
...
def tier_index(k_axis: int, k_rows: int, *, skip: int = 0,
    ...
    if k_rows > k_axis - skip:
        raise SystemExit(f"tier rows {k_rows} + skip {skip} exceed axis {k_axis}")
...
def sparse_tables(experts: int, k_rows: int, sparsity: float, *,
    ...
    if not 0.0 <= sparsity <= 1.0:
        raise SystemExit(f"--sparsity must be in [0, 1], got {sparsity}")
    if k_rows % PAIR_GROUP:
        raise SystemExit(f"tier rows must be even (pair group), got {k_rows}")
...
def summarize(samples: Sequence[float]) -> dict:
    """launch당 µs 표본 → 리포트 필드. median을 대표값으로 쓴다 (공유 머신의
...
def replay_stats(graph: torch.cuda.CUDAGraph, reps: int, *,
                 replays: int = 20, warmup: int = 3) -> dict:
...
def graph_stats(launch: Callable[[int], None], reps: int, *,
                replays: int = 20, error_mode: str = "thread_local") -> dict:
    graph = capture(launch, reps, error_mode=error_mode)
    try:
        return replay_stats(graph, reps, replays=replays)
    finally:
...
def host_stats(step: Callable[[int], None], reps: int, *, replays: int = 20,
               sync_cuda: bool = False) -> dict:
    """host 루프 타이머 — CUDA graph에 담을 수 없는 경로(cold 단독, eager 교차
    검증)용. sync_cuda면 라운드 끝에서 GPU까지 기다린다 (겹침 측정)."""
    for i in range(reps):  # warmup 1라운드
        step(i)
    if sync_cuda:
        torch.cuda.synchronize()
    per = []
...
def env_stamp(device: Optional[torch.device] = None) -> dict:
    ...
    if torch.cuda.is_available():
        ...
        out["gpu"] = {
            ...
            "mem_used_gb": round(torch.cuda.mem_get_info(idx)[1] / 1e9
                                 - torch.cuda.mem_get_info(idx)[0] / 1e9, 1),
        }
...
def emit(payload: dict, out: Optional[str]) -> None:
...
def select_device(index: int) -> torch.device:
    if not torch.cuda.is_available():
        raise SystemExit("CUDA required")
    if index >= torch.cuda.device_count():
        raise SystemExit(f"--device {index} >= {torch.cuda.device_count()}")
