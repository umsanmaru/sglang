#!/usr/bin/env bash
# kt-kernel을 레인 env(torch 2.13/cu130)용으로 재빌드. 소스는 포크와 동일 커밋을 쓴다.
#
#   ENV=prism-glm   bash scripts/build_kt_for_lane_env.sh     # GLM 레인
#   ENV=prism-dense bash scripts/build_kt_for_lane_env.sh     # dense 레인
#
# env마다 따로 빌드해야 하는 이유: kt_kernel 확장이 그 env의 torch ABI와 conda의
# libhwloc/libnuma에 링크된다. 두 레인이 같은 torch(2.13.0+cu130)를 쓰더라도
# site-packages가 다르므로 설치는 각각이다.
set -euo pipefail
ENV=${ENV:?대상 env (prism-glm | prism-dense)}
